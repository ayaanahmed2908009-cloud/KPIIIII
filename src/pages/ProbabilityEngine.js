...
export default function ProbabilityEngine({ analysisHistory }) {
  // Start on the most recent analysis
  const [selectedIndex, setSelectedIndex] = useState(() =>
    analysisHistory.length > 0 ? analysisHistory.length - 1 : 0
  );
    ...
    return (
      <div style={{
        ...
        <div style={{ textAlign: 'center', maxWidth: '420px', padding: '40px 24px' }}>
          ...
          <h2 style={{ color: '#f1f5f9', fontSize: '20px', fontWeight: '700', marginBottom: '10px' }}>
            No analyses yet
          </h2>
          <p style={{ color: '#64748b', fontSize: '14px', lineHeight: '1.6' }}>
            Run an AI analysis from the Dashboard to start building the Probability Engine history.
          </p>
        </div>
      </div>
    );
  ...
  const entry = analysisHistory[selectedIndex];
  const { analysis, weekNumber, dateRun } = entry;
  ...
  const isLatest = selectedIndex === analysisHistory.length - 1;
    ...
    <div style={{
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto' }}>
        ...
        <div style={{ marginBottom: '24px' }}>
          <h1 style={{ color: '#f1f5f9', fontSize: '22px', fontWeight: '800', margin: '0 0 4px' }}>
            Probability Engine
          </h1>
          <p style={{ color: '#475569', fontSize: '13px', margin: 0 }}>
            AI-powered organisation-wide target probability — browse all analysis runs
          </p>
        </div>
        ...
        <div style={{
          ...
          <button
            ...
            style={{
              background: selectedIndex === 0 ? '#0f172a' : '#334155',
              border: '1px solid #475569',
              borderRadius: '8px',
              color: selectedIndex === 0 ? '#334155' : '#e2e8f0',
                ...
                >
                  ...
                  {i === analysisHistory.length - 1 && (
                    <span style={{
                      ...
                      padding: '1px 5px', borderRadius: '999px', fontWeight: '700'
                    }}>Latest</span>
                  )}
                </button>
              );
          ...
          <button
            ...
            disabled={isLatest}
            style={{
              background: isLatest ? '#0f172a' : '#334155',
              border: '1px solid #475569',
              borderRadius: '8px',
              color: isLatest ? '#334155' : '#e2e8f0',
              width: '36px', height: '36px',
              display: 'flex', alignItems: 'center', justifyContent: 'center',
        ...
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'flex-start', gap: '20px' }}>
            <div style={{ flex: 1 }}>
              ...
              <h2 style={{ fontSize: '24px', fontWeight: '800', color: '#f1f5f9', marginBottom: '4px', lineHeight: 1.2 }}>
          ...
          <div style={{ marginTop: '24px', paddingTop: '20px', borderTop: '1px solid #0f172a' }}>
            ...
            <div style={{ marginTop: '14px', height: '8px', borderRadius: '4px', background: '#0f172a', overflow: 'hidden', display: 'flex' }}>
              <div style={{ width: `${(onTrackCount / totalKPIs) * 100}%`, background: '#34d399', transition: 'width 0.4s' }} />
              <div style={{ width: `${(atRiskCount / totalKPIs) * 100}%`, background: '#fbbf24', transition: 'width 0.4s' }} />
              <div style={{ width: `${(criticalCount / totalKPIs) * 100}%`, background: '#f87171', transition: 'width 0.4s' }} />
            </div>
          </div>
        </div>
