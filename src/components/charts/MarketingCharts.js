...
function buildMarketingData(history) {
  if (!hasEnoughData(history, 'marketing')) return { data: PLACEHOLDER.marketing, isPlaceholder: true };
...
export function FollowerGrowthChart({ history }) {
  const { data, isPlaceholder } = buildMarketingData(history);
  ...
  const insight = latest
    ? `Currently ${latest.followers.toLocaleString()} followers — ${(3000 - latest.followers).toLocaleString()} away from target. At +${weeklyGain}/week, target reached in ~${weeksToTarget} weeks.`
    : '';

  return (
    <ChartCard title="Follower Growth Trajectory" subtitle="Weekly cumulative vs. pace-to-3,000 line" color={COLOR} isPlaceholder={isPlaceholder} insight={insight}>
      <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
        <StatPill label="Current" value={(latest?.followers || 0).toLocaleString()} color={COLOR} />
        <StatPill label="Target" value="3,000" color="#475569" />
        <StatPill label="Gap" value={(3000 - (latest?.followers || 0)).toLocaleString()} color="#fbbf24" />
      </div>
...
export function EngagementGaugeChart({ history }) {
  ...
  return (
    <ChartCard title="Engagement Rate" subtitle="Current week gauge + 4-week rolling average" color={COLOR} isPlaceholder={isPlaceholder}
...
export function ContentMixChart({ history }) {
  ...
  return (
    <ChartCard title="Content Mix — Posts vs. Videos" subtitle="Weekly stacked bar · target ratio 2:1" color={COLOR} isPlaceholder={isPlaceholder}
...
export function PressFunnelChart({ history }) {
  ...
  return (
    <ChartCard title="Press Pipeline Funnel" subtitle="Cumulative YTD outreach → confirmed mentions" color={COLOR} isPlaceholder={isPlaceholder}
...
export function MomentumCard({ history }) {
  ...
  return (
    <ChartCard title="AI Momentum Assessment" subtitle="4-week rolling analysis of growth & engagement" color={COLOR} isPlaceholder={isPlaceholder} isAI>
