...
import { ChartCard, DarkTooltip, SemiGauge, AIInsightBox, StatPill, CHART_DEFAULTS } from './ChartShared';
import { PLACEHOLDER, hasEnoughData, extractTeamData } from '../../data/placeholderData';

const COLOR = '#8B5CF6';

function buildData(history) {
  if (!hasEnoughData(history, 'generalManagement')) return { data: PLACEHOLDER.generalManagement, isPlaceholder: true };
...
// 1 — Headcount Line Chart with Join/Leave events
export function HeadcountChart({ history }) {
  const { data, isPlaceholder } = buildData(history);
  const latest = data[data.length - 1];
  const netChange = latest && data.length > 1 ? latest.total - data[0].total : 0;
  ...
  return (
    <ChartCard title="Team Headcount" subtitle="Weekly total · joins (▲) and departures (▼) overlaid" color={COLOR} isPlaceholder={isPlaceholder}
      insight={`${latest?.total || 14} active members (target: 22, baseline: 14). Net change: +${netChange} in ${data.length} weeks (+${weeklyRate}/week avg). ~${weeksToTarget} weeks to target.`}>
      <div style={{ display: 'flex', gap: '10px', marginBottom: '12px' }}>
        <StatPill label="Current" value={latest?.total || 14} color={COLOR} />
      ...
      <ResponsiveContainer width="100%" height={180}>
        <ComposedChart data={data} margin={CHART_DEFAULTS.margin}>
          ...
          <YAxis tick={{ fill: '#475569', fontSize: 10 }} domain={[12, 26]} />
          ...
          <ReferenceLine y={14} stroke="#334155" strokeDasharray="4 4" label={{ value: 'Baseline 14', fill: '#475569', fontSize: 10 }} />
...
export function OKRHeatmapChart({ history }) {
  ...
  return (
    <ChartCard title="OKR Completion Heatmap" subtitle="Each cell = one week · green >70% · amber 50–70% · red <50%" color={COLOR} isPlaceholder={isPlaceholder}
      ...
      <ResponsiveContainer width="100%" height={100}>
        <BarChart data={data.map(d => ({ week: d.week, rate: d.okrDue > 0 ? parseFloat((d.okrCompleted / d.okrDue * 100).toFixed(1)) : 0 }))} margin={{ top: 0, right: 5, left: -20, bottom: 0 }}>
          ...
          <Bar dataKey="rate" radius={[3, 3, 0, 0]} name="Completion" fill={COLOR + '80'}>
            {data.map((d, i) => {
              const rate = d.okrDue > 0 ? (d.okrCompleted / d.okrDue) * 100 : 0;
              return <rect key={i} fill={rate >= 70 ? '#34d399' : rate >= 50 ? '#fbbf24' : '#f87171'} />;
            })}
          </Bar>
        </BarChart>
      </ResponsiveContainer>
...
export function RetentionDonutChart({ history }) {
  ...
  return (
    <ChartCard title="Member Retention Rate" subtitle="Rolling retention vs. 80% annual target" color={COLOR} isPlaceholder={isPlaceholder}
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '20px' }}>
        ...
        <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '8px' }}>
          ...
          <div style={{ height: '6px', background: '#0f172a', borderRadius: '3px' }}>
            <div style={{ height: '100%', width: `${retentionRate}%`, background: onTarget ? '#10B981' : '#F59E0B', borderRadius: '3px' }} />
          </div>
        </div>
      </div>
    </ChartCard>
...
// 4 — Worker Satisfaction Survey Bars
export function SatisfactionChart({ history }) {
  ...
  const surveyData = data.filter(d => d.satisfaction > 0).map(d => ({
    ...
    lowSample: d.respondents < 10
  }));
  const latest = surveyData[surveyData.length - 1];

  return (
    <ChartCard title="Worker Satisfaction Survey" subtitle="Quarterly results · ≥90% target · ⚠ = low response count" color={COLOR} isPlaceholder={isPlaceholder}
      insight={surveyData.length > 0
        ? `Latest: ${latest.satisfaction}% (n=${latest.respondents}). ${latest.lowSample ? '⚠ Low response count — treat with caution.' : ''} ${latest.satisfaction >= 90 ? '✓ On target.' : `${90 - latest.satisfaction} points below 90% target.`}`
        : 'No survey data yet. Enter quarterly results in the weekly input form.'}>
      {surveyData.length === 0 ? (
        <div style={{ textAlign: 'center', padding: '32px', color: '#475569', fontSize: '13px' }}>No quarterly survey data submitted yet.</div>
      ) : (
        <>
          <ResponsiveContainer width="100%" height={170}>
            <BarChart data={surveyData} margin={CHART_DEFAULTS.margin}>
              ...
              <Bar dataKey="satisfaction" radius={[4, 4, 0, 0]} name="Satisfied %">
                {surveyData.map((d, i) => (
                  <rect key={i} fill={d.lowSample ? '#F59E0B80' : d.satisfaction >= 90 ? '#10B981' : '#8B5CF6'} />
                ))}
              </Bar>
            </BarChart>
          </ResponsiveContainer>
...
export function SelfVsAIChart({ history, analysisHistory }) {
  ...
  return (
    <ChartCard title="Self-Assessment vs AI Probability" subtitle="Weekly self-assessed score vs Claude's calculated probability" color={COLOR} isPlaceholder={isPlaceholder}
      insight={avgDivergence !== null ? `Average divergence: ${avgDivergence} points. ${avgDivergence > 15 ? '⚠ High divergence — team perception and AI assessment differ significantly.' : '✓ Estimates are well-aligned.'}` : `Self-assessed this week: ${latestSelf}%. Run AI analysis to compare against calculated probability.`}>
